...
### set up workspace (Hunter Mining — Rehab/Restoration) ###
...
# Output directories
hmsc_dir <- file.path(proj_root, "models/hunter/hmsc_rehab")
...
selected_vars <- ordistep_selected_vars$hunter_restoration

# ADD read depth
log_reads <- hunter_otus %>%
  filter(hunter_preds$Treat == "Rehab") %>%
  rowSums() %>%
  log10()
...
XY <- hunter_xy %>% filter(hunter_preds$Treat == "Rehab")
