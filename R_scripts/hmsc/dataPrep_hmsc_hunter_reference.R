...
### set up workspace (Hunter Mining — Reference) ###
...
# Output directories
hmsc_dir <- file.path(proj_root, "models/hunter/hmsc_reference")
...
selected_vars <- ordistep_selected_vars$hunter_reference

# ADD read depth
log_reads <- hunter_otus %>%
  filter(hunter_preds$Treat == "Reference") %>%
  rowSums() %>%
  log10()
...
X <- hunter_preds %>%
  ...
  mutate(read_count = log_reads) # Add readcount
# study design and coords
XY <- hunter_xy %>% filter(hunter_preds$Treat == "Reference")
...
models[[1]] <- Hmsc(
  ...
  studyDesign = XY %>% select(Plot_ID) %>% mutate(Plot_Spatial = Plot_ID) %>% mutate(across(everything(), as_factor)),
  ranLevels = list(Plot_ID = rL1, Plot_Spatial = rL0),
  distr = "probit",
  YScale = TRUE
)
