...
### set up workspace (Snowy Hydro — Rehab/Restoration) ###
...
# Output directories
hmsc_dir <- file.path(proj_root, "models/snowies/hmsc_rehab")
...
selected_vars <- ordistep_selected_vars$snowy_restoration

# ADD read depth
log_reads <- snowies_otus %>%
  filter(snowies_preds$Treat == "Rehab") %>%
  rowSums() %>%
  log10()
...
XY <- snowies_xy %>% filter(snowies_preds$Treat == "Rehab")
...
# Create spatial data at plot level
spatial_data <- XY %>%
  group_by(Plot_ID) %>%
  slice(1) %>%
  ungroup() %>%
...
rL0 <- HmscRandomLevel(sData = spatial_data, longlat = TRUE)
...
mean(MF1$TjurR2, na.rm = T) # 0.17
mean(MF1$AUC, na.rm = T) # 0.89
